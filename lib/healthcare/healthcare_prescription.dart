...
class _HealthcarePrescriptionState extends State<HealthcarePrescription> {
  ...
  Future<void> _clearSlot({int? index}) async {
    ...
    try {
      ...
      if (mounted && index == null) _showMsg("Course stopped from today. Prescription records archived.", Colors.blueGrey);
    } catch (e) {
  ...
  Future<void> _saveChanges() async {
    ...
    try {
      ...
      await batch.commit();
      _showMsg("Prescription updated. Synced cleanly with analytics database.", Colors.teal);
    } catch (e) {
        ...
        Expanded(
          ...
          child: ElevatedButton(
            ...
            child: Text(
              isLockedOther 
                  ? "BIN UNAVAILABLE" 
                  : (isSlotEmpty ? "Sync Slot" : "Update Prescription"), 
